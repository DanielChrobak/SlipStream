...
struct EncoderGPUMetrics {
    std::atomic<int64_t> minWaitUs{INT64_MAX};
    std::atomic<int64_t> maxWaitUs{0};
    std::atomic<int64_t> sumWaitUs{0};
    std::atomic<uint64_t> waitCount{0};
    std::atomic<uint64_t> timeoutCount{0};
    std::atomic<uint64_t> noWaitCount{0};
    ...
    void RecordWait(int64_t waitUs, bool timedOut) {
        ...
        // Update minimum
        for (int64_t m = minWaitUs.load();
             waitUs < m && !minWaitUs.compare_exchange_weak(m, waitUs););

        // Update maximum
        for (int64_t m = maxWaitUs.load();
             waitUs > m && !maxWaitUs.compare_exchange_weak(m, waitUs););
    }

    struct Snapshot {
        int64_t minUs, maxUs, avgUs;
        uint64_t count, timeouts, noWait;
    };

    Snapshot GetAndReset() {
        Snapshot s{
            minWaitUs.exchange(INT64_MAX),
            maxWaitUs.exchange(0),
            0,
            waitCount.exchange(0),
            timeoutCount.exchange(0),
            noWaitCount.exchange(0)
        };

        int64_t sum = sumWaitUs.exchange(0);
        s.avgUs = s.count > 0 ? sum / static_cast<int64_t>(s.count) : 0;

        if (s.minUs == INT64_MAX) {
            s.minUs = 0;
        }
...
private:
    ...
    std::atomic<uint64_t> lastEncodeFence{0};
    ...
    void InitGPUSync() {
        ...
        if (SUCCEEDED(device->CreateQuery(&qd, &encodeQuery))) {
            LOG("Encoder GPU sync: Query");
        }
    }
    ...
    bool WaitForGPU(uint64_t signalValue, DWORD timeoutMs = GPU_WAIT_TIMEOUT_MS) {
        ...
        if (useFence && encodeFence) {
            if (encodeFence->GetCompletedValue() >= signalValue) {
            ...
            bool completed = (WaitForSingleObject(fenceEvent, timeoutMs) == WAIT_OBJECT_0) ||
                             (encodeFence->GetCompletedValue() >= signalValue);
        ...
        if (encodeQuery) {
            {
                ...
                context->Flush();
                if (context->GetData(encodeQuery, nullptr, 0, 0) == S_OK) {
                    gpuMetrics.RecordWait(0, false);
                    return true;
                }
            }
            ...
            for (DWORD elapsed = 0; elapsed < timeoutMs && !completed; elapsed++) {
                Sleep(1);
                MTLock lock(multithread);
                completed = context->GetData(encodeQuery, nullptr, 0, 0) == S_OK;
            }
    ...
    void ConfigureEncoder(const AVCodec* codec) {
        auto set = [this](const char* k, const char* v) {
            av_opt_set(codecContext->priv_data, k, v, 0);
        };
        ...
        if (!strcmp(codec->name, "av1_nvenc")) {
            ...
            set("crf", "28");
            set("svtav1-params",
                "tune=0:fast-decode=1:enable-overlays=0:scd=0:lookahead=0:lp=1:"
                "tile-rows=0:tile-columns=1:enable-tf=0:enable-cdef=0:"
...
public:
    ...
    AV1Encoder(int w, int h, int fps,
               ID3D11Device* dev, ID3D11DeviceContext* ctx, ID3D11Multithread* mt)
    ...
    ~AV1Encoder() {
        ...
        SafeRelease(encodeFence, context4, device5, encodeQuery,
                    stagingTexture, multithread, context, device);
    }
    ...
    EncodedFrame* Encode(ID3D11Texture2D* texture, int64_t timestamp, bool forceKeyframe = false) {
        ...
        bool needsKeyframe = forceKeyframe ||
                             (steady_clock::now() - lastKeyframe >= KEYFRAME_INTERVAL);
        ...
        if (useHardware) {
            ...
            if (!WaitForGPU(signalValue, GPU_WAIT_TIMEOUT_MS)) {
                failedCount++;
            ...
            for (int y = 0; y < height; y++) {
                memcpy(
                    hwFrame->data[0] + y * hwFrame->linesize[0],
        ...
        // Handle EAGAIN by draining pending packets first
        if (ret == AVERROR(EAGAIN)) {
            while (avcodec_receive_packet(codecContext, packet) == 0) {
                if (packet->flags & AV_PKT_FLAG_KEY) {
                    gotKeyframe = true;
                }
                outputFrame.data.insert(
                    outputFrame.data.end(),
                    packet->data,
    ...
    bool IsEncodeComplete() {
        return !useFence || !encodeFence ||
               encodeFence->GetCompletedValue() >= lastEncodeFence.load();
    }
};

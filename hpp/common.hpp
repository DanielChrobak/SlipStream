...
// Metrics structs for detailed performance monitoring
...
public:
    ...
    bool ValidateSession(const std::string& token) {
        ...
        if (now - it->second.created > SESSION_TIMEOUT || now - it->second.lastActivity > ACTIVITY_TIMEOUT)
            return sessions.erase(it), false;
        return it->second.lastActivity = now, true;
    }

    void InvalidateSession(const std::string& token) { std::lock_guard<std::mutex> lock(mtx); sessions.erase(token); }
    ...
    bool IsAllowed(const std::string& ip) {
        ...
        if (now - it->second.firstAttempt > ATTEMPT_WINDOW) return rateLimits.erase(it), true;

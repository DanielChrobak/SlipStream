...
// ============================================================================
// Multithread Lock Helper
// ============================================================================

struct MTLock {
    ID3D11Multithread* mt;

    MTLock(ID3D11Multithread* m) : mt(m) {
        if (mt) mt->Enter();
    }
...
constexpr int PBKDF2_KEY_LENGTH = 32;
...
inline bool VerifyPassword(const std::string& password, const std::string& salt, const std::string& storedHash) {
    ...
    if (computed.empty() || computed.size() != storedHash.size()) {
        return false;
    }
...
private:
    ...
    struct RateLimit {
        int attempts = 0;
        steady_clock::time_point firstAttempt;
        steady_clock::time_point lockoutUntil;
    };
    ...
    static constexpr auto SESSION_TIMEOUT = hours(24);
    static constexpr auto ACTIVITY_TIMEOUT = hours(4);
    static constexpr auto ATTEMPT_WINDOW = minutes(15);
    static constexpr auto LOCKOUT_DURATION = minutes(30);
...
public:
    std::string CreateSession(const std::string& username, const std::string& clientIP = "") {
        ...
        std::erase_if(sessions, [&](const auto& p) {
            return p.second.username == username;
        });
    ...
    bool ValidateSession(const std::string& token) {
        ...
        if (now - it->second.created > SESSION_TIMEOUT ||
            now - it->second.lastActivity > ACTIVITY_TIMEOUT) {
            sessions.erase(it);
            return false;
        }
    ...
    bool IsAllowed(const std::string& ip) {
        ...
        if (it->second.lockoutUntil > steady_clock::time_point{} && now < it->second.lockoutUntil) {
            return false;
        }

        // Check if attempt window has expired
        if (now - it->second.firstAttempt > ATTEMPT_WINDOW) {
            rateLimits.erase(it);
            return true;
        }
    ...
    void RecordAttempt(const std::string& ip, bool success) {
        ...
        if (r.attempts == 0 || now - r.firstAttempt > ATTEMPT_WINDOW) {
            r = {1, now, {}};
    ...
    int RemainingAttempts(const std::string& ip) {
        ...
        return std::max(0, MAX_ATTEMPTS - it->second.attempts);
    }
    ...
    int LockoutSecondsRemaining(const std::string& ip) {
        ...
        auto it = rateLimits.find(ip);
        if (it == rateLimits.end() || it->second.lockoutUntil <= steady_clock::now()) {
            return 0;
        }

        return static_cast<int>(
            duration_cast<seconds>(it->second.lockoutUntil - steady_clock::now()).count()
        );
    }
    ...
    void Cleanup() {
        ...
        std::erase_if(sessions, [&](const auto& p) {
            return now - p.second.created > SESSION_TIMEOUT ||
                   now - p.second.lastActivity > ACTIVITY_TIMEOUT;
        });

        // Clean up expired rate limits
        std::erase_if(rateLimits, [&](const auto& p) {
            return now - p.second.firstAttempt > ATTEMPT_WINDOW &&
                   p.second.lockoutUntil <= now;
        });
    }
};

...
export const C = {
    ...
    MAX_FRAMES: 6, FRAME_TIMEOUT_MS: 100, AUDIO_RATE: 48000, AUDIO_CH: 2,
    AUDIO_BUF: 0.04, DC: { ordered: false, maxRetransmits: 0 },
    JITTER_TARGET_FRAMES: 1, JITTER_MAX_FRAMES: 2, JITTER_MAX_AGE_MS: 50,
    PRESENT_INTERVAL_WINDOW: 60, CLOCK_OFFSET_SAMPLES: 8, METRICS_LOG_INTERVAL_MS: 1000
};

export const Stage = { IDLE: 'idle', CONNECT: 'connect', AUTH: 'auth', OK: 'connected', ERR: 'error' };

const mkClockSync = () => ({
    offset: 0, offsetSamples: [], rttSamples: [], lastOffset: 0, driftPerSecond: 0,
    lastUpdateMs: 0, valid: false, sampleCount: 0, maxDriftSeen: 0,
    avgRttUs: 0, minRttUs: Infinity, maxRttUs: 0
});

const mkJitterMetrics = () => ({
    queueDepthSum: 0, queueDepthSamples: 0, maxQueueDepth: 0,
    framesDroppedLate: 0, framesDroppedOverflow: 0,
    frameAgeSum: 0, frameAgeSamples: 0, presentIntervals: [], lastPresentTs: 0,
    serverAgeSum: 0, serverAgeSamples: 0, avgQueueDepth: 0, avgFrameAgeMs: 0,
    avgServerAgeMs: 0, intervalStdDev: 0, intervalMean: 0,
    minFrameAgeMs: Infinity, maxFrameAgeMs: 0, minServerAgeMs: Infinity, maxServerAgeMs: 0
});

export const S = {
    pc: null, dc: null, decoder: null, ready: false, needKey: true,
    reinit: false, hwAccel: 'unknown', W: 0, H: 0, rtt: 0,
    hostFps: 60, clientFps: 60, currentFps: 60, currentFpsMode: 0,
    fpsSent: false, authenticated: false, monitors: [], currentMon: 0, tabbedMode: false,
    ...
    stats: {
        recv: 0, dec: 0, rend: 0, bytes: 0, audio: 0, moves: 0, clicks: 0, keys: 0,
        chunksReceived: 0, framesComplete: 0, framesDropped: 0, framesTimeout: 0,
        keyframesReceived: 0, deltaFramesReceived: 0, decodeErrors: 0, renderErrors: 0,
    ...
    sessionToken: null, username: null, presentQueue: [], presentLoopRunning: false,
    clockSync: mkClockSync(), jitterMetrics: mkJitterMetrics(),
    networkMetrics: {
        packetsReceived: 0, videoPackets: 0, controlPackets: 0, audioPackets: 0,
        bytesReceived: 0, minPacketSize: Infinity, maxPacketSize: 0
    },
...
export const getClockSyncStats = () => {
    const cs = S.clockSync;
    return {
        offsetMs: cs.offset / 1000, rttMs: cs.avgRttUs / 1000,
        minRttMs: cs.minRttUs === Infinity ? 0 : cs.minRttUs / 1000,
        maxRttMs: cs.maxRttUs / 1000, driftMsPerSec: cs.driftPerSecond / 1000,
        samples: cs.sampleCount, valid: cs.valid
    };
};

export const resetStats = () => {
    const now = performance.now();
    const result = { ...S.stats, elapsed: now - S.stats.lastUpdate };
...
export const resetJitterMetrics = () => {
    ...
    const result = {
        avgQueueDepth: m.avgQueueDepth, maxQueueDepth: m.maxQueueDepth,
        avgFrameAgeMs: m.avgFrameAgeMs, minFrameAgeMs: m.minFrameAgeMs === Infinity ? 0 : m.minFrameAgeMs,
        maxFrameAgeMs: m.maxFrameAgeMs, avgServerAgeMs: m.avgServerAgeMs,
        minServerAgeMs: m.minServerAgeMs === Infinity ? 0 : m.minServerAgeMs,
        maxServerAgeMs: m.maxServerAgeMs, intervalMean: m.intervalMean, intervalStdDev: m.intervalStdDev,
        framesDroppedLate: m.framesDroppedLate, framesDroppedOverflow: m.framesDroppedOverflow
    };

    Object.assign(m, {
        queueDepthSum: 0, queueDepthSamples: 0, maxQueueDepth: 0,
        framesDroppedLate: 0, framesDroppedOverflow: 0, frameAgeSum: 0, frameAgeSamples: 0,
        serverAgeSum: 0, serverAgeSamples: 0, presentIntervals: [],
        minFrameAgeMs: Infinity, maxFrameAgeMs: 0, minServerAgeMs: Infinity, maxServerAgeMs: 0
    });
    return result;
};
...
export const resetNetworkMetrics = () => {
    ...
    const result = {
        packetsReceived: m.packetsReceived, videoPackets: m.videoPackets,
        controlPackets: m.controlPackets, audioPackets: m.audioPackets,
        bytesReceived: m.bytesReceived, minPacketSize: m.minPacketSize === Infinity ? 0 : m.minPacketSize,
        maxPacketSize: m.maxPacketSize, avgPacketSize: m.packetsReceived > 0 ? m.bytesReceived / m.packetsReceived : 0
    };
...
export const resetDecodeMetrics = () => {
    ...
    const result = {
        decodeCount: m.decodeCount, maxQueueSize: m.maxQueueSize,
...
export const startMetricsLogger = () => {
    ...
    metricsLogInterval = setInterval(() => {
        ...
        const status = S.dc?.readyState === 'open' ? (S.ready ? '%c[LIVE]' : '%c[WAIT]') : '%c[DISC]';
        ...
        if (clock.valid) {
            log('LATENCY', '#eab308', `E2E: ${e2e.toFixed(1)}ms | RTT: ${clock.rttMs.toFixed(1)}ms (${clock.minRttMs.toFixed(1)}/${clock.maxRttMs.toFixed(1)}) | FrameAge: ${jitter.avgServerAgeMs.toFixed(1)}ms (${jitter.minServerAgeMs.toFixed(1)}/${jitter.maxServerAgeMs.toFixed(1)})`);
        }

        if (jitter.avgQueueDepth > 0 || jitter.intervalMean > 0) {
            log('JITTER', '#eab308', `Queue: ${jitter.avgQueueDepth.toFixed(2)} (max:${jitter.maxQueueDepth}) | Interval: ${jitter.intervalMean.toFixed(1)}±${jitter.intervalStdDev.toFixed(1)}ms | LocalAge: ${jitter.avgFrameAgeMs.toFixed(1)}ms`);
        }

        if (decode.decodeCount > 0) {
            log('DECODE', '#eab308', `Count: ${decode.decodeCount} | Time: ${decode.avgDecodeTimeMs.toFixed(2)}ms (${decode.minDecodeTimeMs.toFixed(2)}/${decode.maxDecodeTimeMs.toFixed(2)}) | Queue: ${decode.maxQueueSize} | HW: ${S.hwAccel}`);
        }

        if (render.renderCount > 0) {
            log('RENDER', '#eab308', `Count: ${render.renderCount} | Time: ${render.avgRenderTimeMs.toFixed(2)}ms (${render.minRenderTimeMs.toFixed(2)}/${render.maxRenderTimeMs.toFixed(2)}) | Resolution: ${S.W}x${S.H}`);
        }

        if (network.packetsReceived > 0) {
            log('NETWORK', '#eab308', `Packets: ${network.packetsReceived} (V:${network.videoPackets} A:${network.audioPackets} C:${network.controlPackets}) | Size: ${network.avgPacketSize.toFixed(0)}B avg (${network.minPacketSize}-${network.maxPacketSize})`);
        }

        if (clock.valid && uptimeSeconds % 5 === 0) {
            log('CLOCK', '#eab308', `Offset: ${clock.offsetMs.toFixed(2)}ms | Drift: ${clock.driftMsPerSec.toFixed(3)}ms/s | Samples: ${clock.samples}`);
        }
        ...
        if (totalDrops > 0 || stats.decodeErrors > 0) {
            log('DROPS', '#ef4444', `Network: ${stats.framesDropped} | Timeout: ${stats.framesTimeout} | Late: ${jitter.framesDroppedLate} | Overflow: ${jitter.framesDroppedOverflow} | DecodeErr: ${stats.decodeErrors}`);
        }

        if (stats.moves > 0 || stats.clicks > 0 || stats.keys > 0) {
            log('INPUT', '#eab308', `Mouse: ${stats.moves} moves, ${stats.clicks} clicks | Keys: ${stats.keys}`);
        }

        if (stats.audio > 0) {
            log('AUDIO', '#eab308', `Packets: ${stats.audio} | Enabled: ${S.audioEnabled}`);
        }

        if (uptimeSeconds % 10 === 0) {
            log('SESSION', '#a855f7', `Uptime: ${uptimeSeconds}s | Frames: ${ss.totalFrames} (${(ss.totalFrames / uptimeSeconds).toFixed(1)} avg) | Data: ${(ss.totalBytes / 1048576).toFixed(2)}MB (${(ss.totalBytes * 8 / 1048576 / uptimeSeconds).toFixed(2)} Mbps avg) | Drops: ${ss.totalDrops} | Keys: ${ss.totalKeyframes}`);
        }
    }, C.METRICS_LOG_INTERVAL_MS);
};

export const stopMetricsLogger = () => {
    if (metricsLogInterval) { clearInterval(metricsLogInterval); metricsLogInterval = null; }
};

...
class AudioCapture {
    ...
    void Loop() {
        ...
        while(running.load(std::memory_order_acquire)) {
            if(!capturing.load(std::memory_order_acquire) || !cap ||
            ...
            while(pktLen > 0 && running.load(std::memory_order_acquire) &&
                  capturing.load(std::memory_order_acquire) && cap) {
                ...
                if(data && frames > 0) {
                    ...
                    if(flags & AUDCLNT_BUFFERFLAGS_DATA_DISCONTINUITY) {
                        DBG("AudioCapture: Data discontinuity detected");
                    }
                    if(flags & AUDCLNT_BUFFERFLAGS_TIMESTAMP_ERROR) {
                        DBG("AudioCapture: Timestamp error flag set");
                    }
                }
    ...
    void Process(const float* data, UINT32 frames, int64_t ts) {
        ...
        if(resampler->buf.size() > maxBuf) {
            DBG("AudioCapture: Resampler buffer overflow, dropping %zu samples",
                resampler->buf.size() - FRAME_SZ*ch*2);
            resampler->buf.erase(resampler->buf.begin(),
                                 resampler->buf.begin() + (resampler->buf.size() - FRAME_SZ*ch*2));
        }
        ...
        while(resampler->buf.size() >= (size_t)(FRAME_SZ*ch)) {
            ...
            if(bytes < 0) {
                if(bytes != OPUS_BUFFER_TOO_SMALL) {
                    WARN("AudioCapture: Opus encode error: %s", opus_strerror(bytes));
                }
                continue;
            }
...
public:
    AudioCapture() {
        ...
        chk(dev->Activate(__uuidof(IAudioClient), CLSCTX_ALL, nullptr, (void**)&cli),
            "AudioClient activation");
        chk(cli->GetMixFormat(&wf), "GetMixFormat");

        sysRate = wf->nSamplesPerSec;
        ch = std::min((int)wf->nChannels, 2);
        DBG("AudioCapture: System format: %d Hz, %d channels, %d bits",
            sysRate, wf->nChannels, wf->wBitsPerSample);
        ...
        chk(cli->GetService(__uuidof(IAudioCaptureClient), (void**)&cap),

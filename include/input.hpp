...
class InputHandler {
    ...
    static HCURSOR GetStdCursor(int i) {
        ...
        if(!cache[0]) {
            for(int j=0; j<13; j++) cache[j] = LoadCursor(nullptr, ids[j]);
        }
        return cache[i];
    }
    ...
    void ResetWin() {
        ...
        if(now - rateStart.load() >= 1000) {
            ...
            if(droppedM > 0 || droppedC > 0 || droppedK > 0) {
                DBG("InputHandler: Rate limit hit - dropped moves:%d clicks:%d keys:%d",
                    std::max(0, droppedM), std::max(0, droppedC), std::max(0, droppedK));
            }
    ...
    void ToAbs(float nx, float ny, LONG& ax, LONG& ay) {
        ...
        ax = (LONG)((px - GetSystemMetrics(SM_XVIRTUALSCREEN)) * 65535 /
                    GetSystemMetrics(SM_CXVIRTUALSCREEN));
        ay = (LONG)((py - GetSystemMetrics(SM_YVIRTUALSCREEN)) * 65535 /
    ...
    bool IsBlocked(WORD vk, bool down) {
        ...
        if(blocked && down) {
            blockedKeys++;
            DBG("InputHandler: Blocked key VK=0x%02X (Ctrl:%d Alt:%d)",
                vk, ctrlDown.load(), altDown.load());
        }
        return blocked;
    }
    ...
    bool DoSendInput(INPUT* in, UINT count) {
        ...
        if(sent != count) {
            ...
            if(err != 0) {
                DBG("InputHandler: SendInput failed, sent %u/%u, error: %lu", sent, count, err);
            }
            return false;
        }
        return true;
...
public:
    ...
    void Enable() { enabled = true; LOG("InputHandler: Enabled"); }
    ...
    bool GetCurrentCursor(CursorType& out) {
        ...
        for(int i=0; i<13; i++) {
            if(ci.hCursor == GetStdCursor(i)) { nc = (CursorType)i; break; }
        }
        out = nc;

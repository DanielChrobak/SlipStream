...
class InputHandler {
    ...
    int64_t GetTimeMs() const { return duration_cast<milliseconds>(steady_clock::now().time_since_epoch()).count(); }

    void ResetRateLimitWindow() {
        int64_t now = GetTimeMs();
        if (now - rateLimitWindowStart.load() >= 1000)
            rateLimitWindowStart = now, moveThisSecond = clicksThisSecond = keysThisSecond = 0;
    }

    bool CheckRateLimit(std::atomic<int>& counter, int max) {
        ResetRateLimitWindow();
        if (counter.fetch_add(1) >= max) { rateLimitedCount++; return false; }
        return true;
    }
    ...
    bool IsBlockedKeyCombo(WORD vk, bool down) {
        ...
        if (vk == VK_LWIN || vk == VK_RWIN || (ctrlDown && altDown && vk == VK_DELETE)) { blockedCount++; return true; }
        return false;
    }
...
public:
    ...
    void MouseMove(float nx, float ny) {
        if (!enabled || !CheckRateLimit(moveThisSecond, MAX_MOVES)) return;
    ...
    void MouseButton(uint8_t button, bool down) {
        if (!enabled || button > 4 || !CheckRateLimit(clicksThisSecond, MAX_CLICKS)) return;
    ...
    void MouseWheel(int16_t dx, int16_t dy) {
        if (!enabled || !CheckRateLimit(clicksThisSecond, MAX_CLICKS)) return;
    ...
    void Key(uint16_t jsKey, uint16_t scanCode, bool down) {
        if (!enabled || !CheckRateLimit(keysThisSecond, MAX_KEYS)) return;
    ...
    bool HandleMessage(const uint8_t* data, size_t len) {
        ...
        switch (magic) {
            case MSG_MOUSE_MOVE: if (len >= sizeof(MouseMoveMsg)) { auto* m = reinterpret_cast<const MouseMoveMsg*>(data); MouseMove(m->x, m->y); return true; } break;
            case MSG_MOUSE_MOVE_REL: if (len >= sizeof(MouseMoveRelMsg)) { auto* m = reinterpret_cast<const MouseMoveRelMsg*>(data); MouseMoveRelative(m->dx, m->dy); return true; } break;
            case MSG_MOUSE_BTN: if (len >= sizeof(MouseBtnMsg)) { auto* m = reinterpret_cast<const MouseBtnMsg*>(data); MouseButton(m->button, m->action != 0); return true; } break;
            case MSG_MOUSE_WHEEL: if (len >= 8) { auto* m = reinterpret_cast<const MouseWheelMsg*>(data); MouseWheel(m->deltaX, m->deltaY); return true; } break;
            case MSG_KEY: if (len >= sizeof(KeyMsg)) { auto* m = reinterpret_cast<const KeyMsg*>(data); Key(m->keyCode, m->scanCode, m->action != 0); return true; } break;
        }
        return false;
    }

    struct Stats { uint64_t moves, clicks, keys, blocked, rateLimited; };
    Stats GetStats() { return {moveCount.exchange(0), clickCount.exchange(0), keyCount.exchange(0), blockedCount.exchange(0), rateLimitedCount.exchange(0)}; }
};

...
inline WORD JsKeyToVK(uint16_t k) {
    ...
    static const std::unordered_map<uint16_t,WORD> m={
        {8,VK_BACK},{9,VK_TAB},{13,VK_RETURN},{16,VK_SHIFT},{17,VK_CONTROL},{18,VK_MENU},{19,VK_PAUSE},{20,VK_CAPITAL},
        {27,VK_ESCAPE},{32,VK_SPACE},{33,VK_PRIOR},{34,VK_NEXT},{35,VK_END},{36,VK_HOME},{37,VK_LEFT},{38,VK_UP},
        {39,VK_RIGHT},{40,VK_DOWN},{44,VK_SNAPSHOT},{45,VK_INSERT},{46,VK_DELETE},{91,VK_LWIN},{92,VK_RWIN},
        {96,VK_NUMPAD0},{97,VK_NUMPAD1},{98,VK_NUMPAD2},{99,VK_NUMPAD3},{100,VK_NUMPAD4},{101,VK_NUMPAD5},
        {102,VK_NUMPAD6},{103,VK_NUMPAD7},{104,VK_NUMPAD8},{105,VK_NUMPAD9},{106,VK_MULTIPLY},{107,VK_ADD},
        {109,VK_SUBTRACT},{110,VK_DECIMAL},{111,VK_DIVIDE},{112,VK_F1},{113,VK_F2},{114,VK_F3},{115,VK_F4},
        {116,VK_F5},{117,VK_F6},{118,VK_F7},{119,VK_F8},{120,VK_F9},{121,VK_F10},{122,VK_F11},{123,VK_F12},
        {144,VK_NUMLOCK},{145,VK_SCROLL},{186,VK_OEM_1},{187,VK_OEM_PLUS},{188,VK_OEM_COMMA},{189,VK_OEM_MINUS},
        {190,VK_OEM_PERIOD},{191,VK_OEM_2},{192,VK_OEM_3},{219,VK_OEM_4},{220,VK_OEM_5},{221,VK_OEM_6},{222,VK_OEM_7}
    };
    auto it=m.find(k);
...
class InputHandler {
    ...
    void InitCur() {
        LPCTSTR ids[]={IDC_ARROW,IDC_IBEAM,IDC_HAND,IDC_WAIT,IDC_APPSTARTING,IDC_CROSS,IDC_SIZEALL,
                       IDC_SIZEWE,IDC_SIZENS,IDC_SIZENWSE,IDC_SIZENESW,IDC_NO,IDC_HELP};
        for(int i=0;i<13;i++) stdCur[i]=LoadCursor(nullptr,ids[i]);
    }
    ...
    bool ChkLim(std::atomic<int>& c,int max) { ResetWin(); return c.fetch_add(1)<max; }
...
public:
    ...
    void Enable() { enabled=true; InitCur(); }
    ...
    bool GetCurrentCursor(CursorType& out) {
        ...
        for(int i=0;i<13;i++) if(ci.hCursor==stdCur[i]) { nc=(CursorType)i; break; }
        out=nc;
    ...
    void MouseMove(float nx,float ny) {
        if(!enabled||!ChkLim(moveCnt,MAX_MV)) return;
    ...
    void MouseButton(uint8_t btn,bool down) {
        ...
        INPUT in{INPUT_MOUSE}; in.mi.dwFlags=fl[btn][down?1:0];
        if(btn>=3) in.mi.mouseData=btn==3?XBUTTON1:XBUTTON2;
        SendInput(1,&in,sizeof(INPUT));
    }

    void MouseWheel(int16_t dx,int16_t dy) {
        if(!enabled||!ChkLim(clickCnt,MAX_CL)) return;
    ...
    void Key(uint16_t jsKey,uint16_t scan,bool down) {
        if(!enabled||!ChkLim(keyCnt,MAX_KY)) return;
    ...
    std::string GetClipboardText() {
        ...
        if(!pW) { CloseClipboard(); return ""; }
        int len=WideCharToMultiByte(CP_UTF8,0,pW,-1,nullptr,0,nullptr,nullptr);
        if(len<=0) { GlobalUnlock(hData); CloseClipboard(); return ""; }

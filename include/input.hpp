...
class InputHandler {
    ...
    std::atomic<uint64_t> droppedMoves{0}, droppedClicks{0}, droppedKeys{0}, blockedKeys{0};

    static constexpr int MAX_MV=500, MAX_CL=50, MAX_KY=100;

    bool CheckLimit(std::atomic<int>& cnt, int max, std::atomic<uint64_t>& dropped);
...
public:
    ...
    [[nodiscard]] bool GetCurrentCursor(CursorType& out);
    void WiggleCenter();
    ...
    [[nodiscard]] bool HandleMessage(const uint8_t* data, size_t len);
    [[nodiscard]] bool SetClipboardText(const std::string& text);

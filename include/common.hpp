...
inline int64_t GetTimestamp() {
    ...
    return static_cast<int64_t>((u.QuadPart - 116444736000000000ULL) / 10);
}
...
public:
    ...
    LinearResampler(int src, int dst, int ch)
        : srcRate(src), dstRate(dst), channels(ch), ratio(static_cast<double>(src) / dst), prev(ch, T{}) {
        buf.reserve(480 * ch * 8);
    }

    void Reset() {
        accum = 0.0;
        std::fill(prev.begin(), prev.end(), T{});
        buf.clear();
    }
    ...
    void ProcessMono(const T* in, size_t frames, int outCh) {
        if (!frames) return;
        double srcRatio = static_cast<double>(dstRate) / srcRate;
        while (accum < frames) {
            size_t i0 = static_cast<size_t>(accum), i1 = i0 + 1;
            double f = accum - i0;
            T s0 = (i0 == 0 && accum < 1.0) ? prev[0] : in[i0];
            T s1 = (i1 < frames) ? in[i1] : s0;
            T s = static_cast<T>(s0 + (s1 - s0) * f);

...
private:
    ...
    uint64_t Signal() {
        if(useFence && c4 && fence) {
            ...
            if(FAILED(hr)) { WARN("VideoEncoder: Signal failed (0x%08X)", hr); return 0; }
            lastSig = fVal;
            return fVal;
        }
        return 0;
    ...
    void Configure() {
        auto set = [this](const char* k, const char* v) {
            ...
            if(ret < 0) { DBG("VideoEncoder: av_opt_set(%s=%s) failed: %s", k, v, AvErr(ret)); }
        };
        ...
        switch(vendor) {
            case GPUVendor::NVIDIA:
                ...
                set("surfaces", "4");
                if(codec == CODEC_H264) {
                    set("cq", "23"); set("forced-idr", "1"); set("strict_gop", "1");
                } else if(codec == CODEC_H265) {
                    set("cq", "25"); set("forced-idr", "1"); set("strict_gop", "1");
                } else {
                    set("cq", "28");
                }
                break;
    ...
    bool TryInitEncoder(GPUVendor v, CodecType cc) {
        ...
        if(!InitHwCtx(enc)) { avcodec_free_context(&cctx); return false; }
...
public:
    VideoEncoder(int width, int height, int fps, ID3D11Device* d, ID3D11DeviceContext* c,
        ...
        for(GPUVendor v : GetVendorPriorityList(DetectGPUVendor(dev))) {
            if(TryInitEncoder(v, cc)) { success = true; break; }
        }
    ...
    GPUVendor GetVendor() const { return vendor; }
    ...
    EncodedFrame* Encode(ID3D11Texture2D* tex, int64_t ts, bool forceKey=false) {
        ...
        if(!tex) { WARN("VideoEncoder: Encode called with null texture"); return nullptr; }

...
IMMDevice* MicPlayback::FindDev(const std::string& name) {
    ...
    while(running.load(std::memory_order_acquire)) {
        ...
        if(ds <= 0) {
            ...
            if(decodeErrors.load() % 100 == 1) {
                WARN("MicPlayback: Opus decode error: %s (total errors: %llu)", opus_strerror(ds), decodeErrors.load());
            }
            continue;
        }
        ...
        if(resampler->buf.size() > FRAME_SZ * ch * 10) {
            ...
            bufferOverruns++;
            DBG("MicPlayback: Buffer overrun, dropped %zu samples (total overruns: %llu)", toDrop/ch, bufferOverruns.load());
        }
    }
    CoUninitialize();
...
MicPlayback::MicPlayback(const std::string& dn) : targetDev(dn) {
    ...
    chk(dev->Activate(__uuidof(IAudioClient), CLSCTX_ALL, nullptr, (void**)&cli), "IAudioClient activation");
    chk(cli->GetMixFormat(&wf), "GetMixFormat");

    devRate = wf->nSamplesPerSec;
    ch = wf->nChannels;
    DBG("MicPlayback: Device format: %d Hz, %d channels, %d bits, format tag: 0x%04X", devRate, ch, wf->wBitsPerSample, wf->wFormatTag);
...
bool MicPlayback::IsInitialized() const { return init.load(std::memory_order_acquire); }
const std::string& MicPlayback::GetDeviceName() const { return actualDevName; }

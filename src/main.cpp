...
bool ValidateUsername(const std::string& u) {
    ...
    for (char c : u) if (!isalnum(static_cast<unsigned char>(c)) && c != '_' && c != '-') return false;
    return true;
}
...
template<typename F> auto AuthRequired(F h) {
    return [h](const httplib::Request& req, httplib::Response& res) {
        std::string token = ExtractBearerToken(req), username;
        if (token.empty() || !g_jwt.ValidateToken(token, username)) {
            JsonError(res, 401, token.empty() ? "Authentication required" : "Invalid or expired token");
            return;
        }
        h(req, res, username);
    };
...
void HandleAuth(const httplib::Request& req, httplib::Response& res) {
    ...
    if (!g_rateLimiter.IsAllowed(ip)) {
        res.status = 429;
        res.set_content(json{{"error", "Too many attempts"}, {"lockoutSeconds", g_rateLimiter.LockoutSecondsRemaining(ip)}}.dump(), "application/json");
    ...
    if (u != g_config.username || !VerifyPassword(p, g_config.salt, g_config.passwordHash)) {
        ...
        res.status = 401;
        res.set_content(json{{"error", "Invalid credentials"}, {"remainingAttempts", g_rateLimiter.RemainingAttempts(ip)}}.dump(), "application/json");
...
int main() {
    try {
        ...
        srv.Get("/api/session", AuthRequired([](auto&, auto& res, const std::string& username) {
            res.set_content(json{{"valid", true}, {"username", username}}.dump(), "application/json");
        }));

        srv.Post("/api/offer", AuthRequired([&](const httplib::Request& req, httplib::Response& res, const std::string& username) {
            ...
            try {
                ...
                LOG("Offer from %s (%s)", username.c_str(), GetClientIP(req).c_str());
                ...
                LOG("Answer to %s", username.c_str());
        ...
        std::thread cleanThread([&] { while (running && !g_quitting) { std::this_thread::sleep_for(std::chrono::minutes(5)); g_rateLimiter.Cleanup(); } });
        ...
        std::thread([&] { std::this_thread::sleep_for(3s); if (g_quitting) { WARN("Shutdown timeout"); ExitProcess(0); } }).detach();

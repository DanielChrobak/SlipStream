...
LRESULT CALLBACK TrayWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    if (msg == WM_TRAYICON) {
        ...
        if (evt == WM_LBUTTONDBLCLK || evt == WM_LBUTTONUP) {
            if (g_consoleWnd) { ShowWindow(g_consoleWnd, SW_RESTORE); SetForegroundWindow(g_consoleWnd); }
            return 0;
        }
        ...
        if (LOWORD(wParam) == ID_TRAY_OPEN && g_consoleWnd) { ShowWindow(g_consoleWnd, SW_RESTORE); SetForegroundWindow(g_consoleWnd); }
        else if (LOWORD(wParam) == ID_TRAY_QUIT) { g_quitting = true; PostQuitMessage(0); }
        return 0;
...
static void TrayThreadMain() {
    ...
    WNDCLASSW wc{}; wc.lpfnWndProc = TrayWndProc; wc.hInstance = GetModuleHandleW(nullptr); wc.lpszClassName = L"SlipStreamTray";
    RegisterClassW(&wc);
    g_trayWnd = CreateWindowExW(0, wc.lpszClassName, L"", WS_POPUP, 0, 0, 0, 0, nullptr, nullptr, wc.hInstance, nullptr);
    ZeroMemory(&g_nid, sizeof(g_nid));
    g_nid.cbSize = sizeof(g_nid); g_nid.hWnd = g_trayWnd; g_nid.uID = 1;
    g_nid.uFlags = NIF_ICON | NIF_MESSAGE | NIF_TIP; g_nid.uCallbackMessage = WM_TRAYICON;
    g_nid.hIcon = LoadIcon(nullptr, IDI_APPLICATION); wcscpy_s(g_nid.szTip, L"SlipStream Server");
    Shell_NotifyIconW(NIM_ADD, &g_nid); g_nid.uVersion = NOTIFYICON_VERSION_4; Shell_NotifyIconW(NIM_SETVERSION, &g_nid);
    g_trayReady = true;
    MSG msg;
    while (!g_quitting && GetMessageW(&msg, nullptr, 0, 0)) { TranslateMessage(&msg); DispatchMessageW(&msg); }
    Shell_NotifyIconW(NIM_DELETE, &g_nid);
    if (g_trayWnd) { DestroyWindow(g_trayWnd); g_trayWnd = nullptr; }
    g_trayReady = false;
}

void InitTray() { g_trayThread = std::thread(TrayThreadMain); while (!g_trayReady) std::this_thread::sleep_for(std::chrono::milliseconds(1)); }

void CleanupTray() {
    g_quitting = true;
    if (g_trayThreadId) PostThreadMessageW(g_trayThreadId, WM_QUIT, 0, 0);
...
int main() {
    try {
        ...
        if (HANDLE h = GetStdHandle(STD_INPUT_HANDLE); h != INVALID_HANDLE_VALUE) {
            DWORD m = 0; if (GetConsoleMode(h, &m)) SetConsoleMode(h, (m & ~ENABLE_QUICK_EDIT_MODE) | ENABLE_EXTENDED_FLAGS);
        }
        InitTray(); ShowWindow(g_consoleWnd, SW_HIDE);
        ...
        std::thread minimizeWatcher([&] { while (!g_quitting) { if (IsIconic(g_consoleWnd)) ShowWindow(g_consoleWnd, SW_HIDE); std::this_thread::sleep_for(100ms); } });
        ...
        srv.Post("/api/auth", JsonHandler([&](const httplib::Request& req, httplib::Response& res, const json& body) {
            ...
            if (!g_auth.IsAllowed(ip)) {
                res.status = 429; res.set_content(json{{"error", "Too many attempts"}, {"lockoutSeconds", g_auth.LockoutSecondsRemaining(ip)}}.dump(), "application/json");
                WARN("Rate limited: %s", ip.c_str()); return;
            }
        ...
        srv.Post("/api/offer", AuthRequired([&](const httplib::Request& req, httplib::Response& res, const std::string& t) {
            ...
            if (req.body.size() > MAX_SDP) { JsonError(res, 413, "Payload too large"); WARN("Rejected oversized offer: %zu bytes", req.body.size()); return; }
            try {
        ...
        std::thread statsThread([&] {
            ...
            while (running && !g_quitting) {
                std::this_thread::sleep_for(1s); uptime++;
                auto st = rtc->GetStats(); uint64_t enc = 0, encFailed = 0;
                EncoderGPUMetrics::Snapshot encGpu = {}; int encW = 0, encH = 0;
                CodecType encCodec = CODEC_H264;
                { std::lock_guard<std::mutex> lk(encMtx); if (encoder) { enc = encoder->GetEncoded(); encFailed = encoder->GetFailed(); encGpu = encoder->GetGPUMetrics(); encW = encoder->GetWidth(); encH = encoder->GetHeight(); encCodec = encoder->GetCodec(); } }
                ...
                auto inputStats = input.GetStats(); uint64_t audioSent = rtc->GetAudioSent();
        ...
        std::thread watchdog([&] {
            ...
            WARN("Shutdown timeout - forcing exit"); CleanupTray(); ExitProcess(0);
        }); watchdog.detach();

        srvThread.join(); encThread.join(); audioThread.join(); statsThread.join(); cleanThread.join(); minimizeWatcher.join();
        CleanupTray(); LOG("Shutdown complete");

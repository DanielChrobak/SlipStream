...
const WORKLET_CODE = `
class RingBuf {
    constructor(ch, cap) { this.ch = ch; this.cap = cap; this.bufs = []; for (let i = 0; i < ch; i++) this.bufs.push(new Float32Array(cap)); this.rp = 0; this.wp = 0; this.avail = 0; }
    write(cd, f) {
        if (f <= 0) return { w: 0, o: 0 };
        let o = 0; const space = this.cap - this.avail;
        if (f > space) { o = f - space; this.rp = (this.rp + o) % this.cap; this.avail -= o; }
        const tw = Math.min(f, this.cap - this.avail);
        if (tw <= 0) return { w: 0, o };
        for (let c = 0; c < this.ch; c++) { const s = cd[c] || cd[0]; for (let i = 0; i < tw; i++) this.bufs[c][(this.wp + i) % this.cap] = s[i] || 0; }
        this.wp = (this.wp + tw) % this.cap; this.avail += tw; return { w: tw, o };
    }
    read(ob, f) {
        const tr = Math.min(f, this.avail);
        for (let c = 0; c < this.ch; c++) { const o = ob[c]; for (let i = 0; i < f; i++) o[i] = i < tr ? this.bufs[c][(this.rp + i) % this.cap] : 0; }
        if (tr > 0) { this.rp = (this.rp + tr) % this.cap; this.avail -= tr; }
        return tr;
    }
...
class StreamAudioProcessor extends AudioWorkletProcessor {
    ...
    process(i, o) {
        const out = o[0]; if (!out || !out.length) return true;
        const f = out[0].length, bufferMs = (this.rb.len / 48000) * 1000;
        if (this.mute) for (let c = 0; c < out.length; c++) out[c].fill(0);
        else {
...
export const initAudio = async () => {
    ...
    try {
        ...
        try {
            ...
            audioWorkletNode = new AudioWorkletNode(ctx, 'stream-audio-processor', { numberOfInputs: 0, numberOfOutputs: 1, outputChannelCount: [2] });
...
const sendToWorklet = ad => {
    if (!audioWorkletNode || !audioWorkletReady) { ad.close(); return; }
    try {
...
export const toggleAudio = () => {
    ...
    } else { S.audioEnabled = false; btn.classList.remove('on'); txt.textContent = 'Enable'; resetAudioState(); }
};

export const closeAudio = () => {
    if (audioWorkletNode) { audioWorkletNode.disconnect(); audioWorkletNode = null; }
    audioWorkletReady = false; S.audioCtx?.close(); resetAudioState();
};

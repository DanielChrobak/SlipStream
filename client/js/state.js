...
export const S = {
    ...
    W: 0, H: 0, rtt: 0, hostFps: 60, currentFps: 60, currentFpsMode: 0, fpsSent: false,
...
export const startMetricsLogger = () => {
    ...
    metricsLogInterval = setInterval(() => {
        ...
        metricsSubscribers.forEach(cb => { try { cb({ uptime: uptimeSeconds, stats, jitter, clock, network, decode, render, session: { ...ss }, computed: { fps, targetFps, fpsEff, mbps } }); } catch {} });
        ...
        if (clock.valid) log('LATENCY', '#eab308', `RTT: ${clock.rttMs.toFixed(1)}ms (${clock.minRttMs.toFixed(1)}/${clock.maxRttMs.toFixed(1)}) | FrameAge: ${jitter.avgServerAgeMs.toFixed(1)}ms (${jitter.minServerAgeMs.toFixed(1)}/${jitter.maxServerAgeMs.toFixed(1)})`);

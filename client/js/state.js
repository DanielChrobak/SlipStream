...
export const detectCodecs = async () => {
    ...
    if (!window.VideoDecoder) return codecCache = { support, best: { codecId: 0, hardwareAccel: false, codecName: 'H.264' } };
    ...
    const best = support.av1Hw ? { codecId: 1, hardwareAccel: true, codecName: 'AV1' } : support.av1 ? { codecId: 1, hardwareAccel: false, codecName: 'AV1' } : support.h264Hw ? { codecId: 0, hardwareAccel: true, codecName: 'H.264' } : { codecId: 0, hardwareAccel: false, codecName: 'H.264' };
...
export const recordAudioBufferHealth = healthMs => {
    const m = S.audioMetrics;
...
export const startMetricsLogger = () => {
    ...
    metricsLogInterval = setInterval(() => {
        ...
        metricsSubscribers.forEach(cb => { try { cb({ uptime: uptimeSeconds, stats, jitter, clock, network, decode, render, audio, session, computed: { fps, targetFps, fpsEff, mbps } }); } catch (e) { console.warn('[State] Metrics subscriber error:', e.message); } });
...
export const resetSessionStats = () => {
    uptimeSeconds = 0;
    sessionTotalFrames = sessionTotalBytes = sessionTotalDrops = sessionTotalKeyframes = sessionTotalDecodeErrors = 0;
};
